...
class EngineeringApp(QMainWindow):
    ...
    def __init__(self):
        ...
        def custom_paste():
            ...
            # прямая вставка без вызова insert_text (избегаем рекурсии)
            current = self.main_display.text()
            if current in ["0", "Ошибка", "Введите выражение!", "Ошибка синтаксиса"]:
                current = ""
            self.main_display.setText(current + filtered_text)
    ...
    def apply_factorial(self):
        ...
        if not text or text in ["0", "Ошибка", "Ошибка синтаксиса", "Введите выражение!"]:
            ...
            return
        try:
            val = int(float(safe_eval(text, self.angle_mode)))
            if val < 0:
    ...
    def get_grid_step(self, min_val, max_val):
        # автоматическое определение оптимального шага сетки
    ...
    def calculate_main(self):
        ...
        # очищаем ошибки перед новым вычислением
        self.error_label.setText("")
        self.main_display.setStyleSheet("")

        # защита от слишком длинных выражений
        if len(raw) > 200:
        ...
        try:
            ...
            # вычисление выражения через safe_eval
    ...
    def set_buttons_enabled(self, enabled: bool):
        ...
        import time
        if hasattr(self, '_last_enable_time'):
            if time.time() - self._last_enable_time < 0.05:  # 50 мс задержка
                return
    ...
    def draw_graph(self):
        ...
        try:
            ...
            # АВТОМАТИЧЕСКАЯ СЕТКА - шаг подбирается под диапазон
            x_step, x_start, x_end = self.get_grid_step(xmin, xmax)
            y_step, y_start, y_end = self.get_grid_step(ymin, ymax)
    ...
    def clear_graph_error(self):
        """Очистка ошибок в поле графика при вводе текста"""
        self.g_in.setStyleSheet("")
        # Очищаем текст только если там сообщение об ошибке
        current_text = self.g_in.text()
    ...
    def insert_text(self, text):
        ...
        try:
            # защита от слишком длинного выражения
            current = self.main_display.text()
            if len(current) > 100 and text not in ["C", "DEL", "="]:
            ...
            if self.second_func:
                ...
                self.trig_btn.setStyleSheet("")
                self.update_clear_button()
                self.set_buttons_enabled(True)  # ВЕРНУЛИ
                return
